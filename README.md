...
[![GitHub Code Style Action Status](https://img.shields.io/github/actions/workflow/status/wsmallnews/preference/fix-php-code-style-issues.yml?branch=main&label=code%20style&style=flat-square)](https://github.com/wsmallnews/preference/actions?query=workflow%3A"Fix+PHP+code+styling"+branch%3Amain)
...
```css
@source '../../../../vendor/wsmallnews/preference/resources/**/*.blade.php';
```

You can publish and run the migrations with:

```bash
php artisan vendor:publish --tag="preference-migrations"
php artisan migrate
```
...
```bash
php artisan vendor:publish --tag="preference-config"
```

Optionally, you can publish the views using

```bash
php artisan vendor:publish --tag="preference-views"
```
...
```php
$preference = new Wsmallnews\Preference();
echo $preference->echoPhrase('Hello, Wsmallnews!');
```
