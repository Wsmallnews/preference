@php
    ...
    $heading = match ($listType) {
        'preferencer' => $this->getProperty('heading', __('sn-preference::preference.components.follows_title_user')),
        'preferenceable' => $this->getProperty('heading', __('sn-preference::preference.components.follows_title_subject')),
        default => $this->getProperty('heading', __('sn-preference::preference.components.follows_title')),
    };

    $selectAllLabel = count($selected) === $follows->count() && $follows->count() > 0
        ? __('sn-preference::preference.action.deselect_all')
        : __('sn-preference::preference.action.select_all');

    $badgeHtml = '<span class="sn-badge sn-badge-warning text-[10px]">' . __('sn-preference::preference.widget.mutual_follow') . '</span>';
@endphp
        ...
        @if ($follows->isNotEmpty())
            ...
            >
                @foreach ($follows as $preference)
                    @php
                        $hasMutual = $preference->options['followed_at'] ?? false;
                    @endphp
                    ...
                    >
                        ...
                        <div class="flex-1 min-w-0">
                            @if ($listType == 'preferenceable')
                                ...
                                >
                                    @if ($hasMutual)
                                        <x-slot name="badge">{!! $badgeHtml !!}</x-slot>
                                        <x-slot name="extra">{{ $preference->options['followed_at'] }} {{ __('sn-preference::preference.widget.mutual_follow') }}</x-slot>
                                    @endif
                                </x-sn-preference::preferencer>
