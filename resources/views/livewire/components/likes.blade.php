        ...
        <div class="flex items-center justify-between">
            <h3 class="sn-h3-text">
                @if ($listType == 'preferencer')
                    {{ $this->getProperty('heading', __('sn-preference::preference.components.likes_title_user')) }}
                @elseif ($listType == 'preferenceable')
                    {{ $this->getProperty('heading', __('sn-preference::preference.components.likes_title_subject')) }}
                @else
                    {{ $this->getProperty('heading', __('sn-preference::preference.components.likes_title')) }}
                @endif
            </h3>
            ...
            @if ($manageMode)
                <div class="sticky bottom-0 flex items-center justify-between px-4 py-3 bg-white border-t border-gray-200 rounded-b-xl shadow-[0_-2px_8px_rgba(0,0,0,0.04)] -mx-4 -mb-8">
                    ...
                    >
                        ...
                        @if (count($selected) === $likes->count() && $likes->count() > 0)
